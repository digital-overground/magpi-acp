...
const parseFrontmatter = (
  ...
  for (const line of frontmatterBlock.split("\n")) {
    const match = line.match(/^(?<key>\w+):\s*(?<value>.*)$/u)?.groups;
    if (match?.key && match.value !== undefined) {
      frontmatter[match.key] = match.value.trim();
    }
  }

  return { content: remaining, frontmatter };
};

const commandSource = (source: "user" | "project", subdir: string): string => {
  if (subdir) {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        const sourceStr = commandSource(source, subdir);
 ...
 */
export const loadSlashCommands = (cwd: string): FileSlashCommand[] => {
  const commands: FileSlashCommand[] = [];

  const userDir = path.join(homedir(), ".pi", "agent", "prompts");
  const projectDir = path.resolve(cwd, ".pi", "prompts");
 ...
 */
export const toAvailableCommands = (
...
export const substituteArgs = (content: string, args: string[]): string => {
  ...
  result = result.replaceAll(/\$(?<argumentNumber>\d+)/gu, (_match, num) => {
    const idx = Math.trunc(Number(String(num))) - 1;
    return args[idx] ?? "";
  });
 ...
 */
export const expandSlashCommand = (
  text: string,
  fileCommands: FileSlashCommand[]
): string => {
  if (!text.startsWith("/")) {
