...
const bundledExtensionPaths = (): string[] =>
  [
    ...
    throw new PiRpcSpawnError(
      `Could not locate the bundled magpi-acp extension: ${bundle}`
    );
  });
...
export class PiRpcProcess {
  ...
  static async spawn(params: SpawnParams): Promise<PiRpcProcess> {
    ...
    const args = [
      ...
      "rpc",
      "--no-themes",
      ...bundledExtensionPaths().flatMap((extension) => [
        "--extension",
        extension,
      ]),
    ];
  ...
  async navigateTree(
    ...
    try {
      const payload = JSON.stringify({
        customInstructions: navigationOptions.customInstructions,
        entryId,
        summarize: navigationOptions.summarize,
      });
