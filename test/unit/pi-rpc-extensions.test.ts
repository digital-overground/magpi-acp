...
void test(
  "Pi RPC loads both bundled extensions",
  { skip: process.platform === "win32" },
  async (t) => {
    const directory = mkdtempSync(path.join(tmpdir(), "magpi-pi-extensions-"));
    ...
    t.after(() => {
      if (previousArgsPath === undefined) {
        delete process.env.MAGPI_TEST_PI_ARGS_PATH;
      } else {
        process.env.MAGPI_TEST_PI_ARGS_PATH = previousArgsPath;
      }
...
`
    ...
    try {
      ...
      const extensions = args.flatMap((argument, index) => {
        ...
        return argument === "--extension" && extension !== undefined
          ? [extension]
          : [];
      });
      ...
      assert.ok(askUserExtension);
      assert.match(
        treeExtension,
        /(?:pi-tree-extension\.js|pi-extension[/\\]tree\.ts)$/u
      );
      assert.match(
        askUserExtension,
        /(?:pi-ask-user-extension\.js|pi-extension[/\\]ask-user\.ts)$/u
      );
    } finally {
      proc.dispose();
    }
