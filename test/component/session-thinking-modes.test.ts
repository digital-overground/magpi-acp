...
class FakeConn {
  ...
  sessionUpdate(msg: unknown): void {
    this.updates.push(msg);
  }
}

test("MagPiAcpAgent: setSessionMode maps to pi setThinkingLevel + emits current_mode_update", async () => {
  const conn = new FakeConn();
  const agent = new MagPiAcpAgent(conn as never);
  ...
  await assert.rejects(
    () =>
      agent.setSessionMode({ modeId: "invalid", sessionId: "nope" } as never),
    /invalid params/iu
  );
});
